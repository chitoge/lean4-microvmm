...
Published companions: [Blueprint](https://chitoge.github.io/lean4-microvmm/blueprint/) for the theorem/dependency view of the current formalization, and [Lean API docs](https://chitoge.github.io/lean4-microvmm/docs/) for declaration-level documentation.
...
| After activation, queue configuration is immutable | Yes | `virtioQueueConfigurationImmutableAfterActivation_queueNum`, `virtioQueueConfigurationImmutableAfterActivation_queueReady`, `virtioQueueConfigurationImmutableAfterActivation_queueAddress`, `virtioQueueConfigurationLatchedAfterActivation_queueNum`, `virtioQueueConfigurationLatchedAfterActivation_queueReady`, `virtioQueueConfigurationLatchedAfterActivation_queueAddress` | This is one of the strongest current proof stories, but it is important to read the theorem shapes correctly. The `immutable` theorems prove that once the queue is active, successful post-activation queue size, ready, and address writes preserve `preservesLiveQueue`. The CVE-facing `latched` corollaries additionally require `hLatched : queueConfigLatched device` to conclude `queueConfigLatched next`. The implementation sets both `activeQueue` and `latchedQueue` at activation, but the current exported activation theorem still exposes only `queueShapeValid`, so there is not yet one top-level theorem from successful activation all the way to the CVE corollaries. |
...
| All device memory accesses remain within validated guest regions | Partial | `virtioRngAcceptedRequestAccessesStayWithinValidatedRegions`, `virtioRngExecutorReportsPureCompletionTrace`, `virtioRngPublicCompletionRefinesPureCompletionTrace` | For the current accepted virtio-rng request shape, the exported access theorem packages planner-level validated read and write regions for the accepted completion plan. Separate executor-trace theorems show that successful completion writes follow the same plan-derived trace. What is not yet exported is one single runtime theorem packaging those facts into an end-to-end validated-access claim for the live path up to the named raw boundary. |
...
- The implementation's successful activation branch writes the same queue into both `activeQueue` and `latchedQueue`, but `virtioQueueActivationEstablishesAcceptedShape` currently exports only the active-queue witness together with `queueShapeValid`. That is why the public CVE story is still a conditional preservation theorem rather than one exported activation-to-CVE theorem.
...
- There is no current single exported end-to-end CVE theorem that starts from successful activation, establishes `queueConfigLatched`, and then carries that fact through the Linux runtime-facing path to the named raw boundary. The implementation and lower proofs strongly suggest that story, but the activation-to-latching bridge is not yet surfaced as one public theorem.
- There is no proof of the raw shim itself.

## Reading path

1. Use the published [Blueprint](https://chitoge.github.io/lean4-microvmm/blueprint/) for the proof narrative and theorem graph, and the published [Lean API docs](https://chitoge.github.io/lean4-microvmm/docs/) when you want declaration-level detail.
2. Read `Microvmm/Proof.lean` for the top-level theorem names.
3. Use the table above as the proof-oriented index, then read [virtio](virtio.md) for the executable contract and fuller narrative around each claim.
4. Read [proof-roadmap](proof-roadmap.md) for the detailed module-by-module proof surface.
5. Use [architecture](architecture.md) and [module-map](module-map.md) when you need to jump from a theorem to the owning implementation module.
6. Use [guest-linux](guest-linux.md) for the documented Linux/initrd runtime contract that the Linux-facing theorems talk about.
