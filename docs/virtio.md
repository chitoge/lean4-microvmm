...
The current proof surface now makes the post-activation preservation part of that hardening argument explicit: once the queue is active, later queue-register rewrites preserve the live queue, and the CVE-facing corollaries show that an already-latched queue stays latched across those rewrites.
The implementation also sets `activeQueue` and `latchedQueue` to the same queue at successful activation. But the exported activation theorem still exposes only `queueShapeValid`, so the repo should not yet be described as having one single top-level end-to-end CVE theorem from activation through the runtime-facing path to the raw boundary.
...
- `Microvmm/Proof/Virtio/Cve2026_5747.lean` packages those facts into conditional mutation-after-activation corollaries for the Firecracker CVE-2026-5747 bug class: given an already-latched state, post-activation queue rewrites preserve `queueConfigLatched`.
...
That last point matters because the virtio proofs no longer stop at an abstract plan or only at transport wrapper helpers. The active VM execution path now goes through decoded MMIO/PCI write helpers, then the standalone or Linux explicit-access shell, then a one-step MMIO-exit/run-loop witness, and then a report-producing executor whose concrete write trace is proved to flow through the Lean `guestWriteU*` wrappers. The repo now also names the remaining raw contract explicitly instead of leaving it implicit. On that successful virtio execution path, the remaining trusted base is the raw FFI/shim boundary and Lean runtime semantics, not an unproved Lean control-flow layer above it. What remains unproved is liveness across fully general exit traces, the raw shim behavior itself, and one single exported activation-to-CVE theorem that packages the activation-side latching fact together with the runtime-facing path.
...
| After activation, queue configuration is immutable | Yes | `Microvmm/Proof/Virtio/Core.lean`, `Microvmm/Proof/Virtio/Cve2026_5747.lean`, `Microvmm/Proof/Virtio/{Mmio,Pci}.lean` | This is one of the strongest current proof stories, but the theorem shapes matter. The shared core theorems prove that once the queue is active, successful post-activation queue size, ready, and address writes preserve `preservesLiveQueue`, and the transport proofs lift that fact through the real MMIO and PCI entry points. The CVE-facing corollaries additionally assume a starting `queueConfigLatched` state. The implementation latches at activation, but the exported activation theorem still exposes only `queueShapeValid`, so the repo does not yet export a single top-level activation-to-CVE theorem. |
...
| All device memory accesses remain within validated guest regions | Partial | `Microvmm/Common.lean`, `Microvmm/Device/Virtio/Rng.lean`, `Microvmm/Proof/Virtio/{Rng,RngExecution}.lean`, `Microvmm/Proof/Kvm/{Resource,RawBoundary}.lean` | For the current accepted virtio-rng request shape, `Microvmm/Proof/Virtio/Rng.lean` packages planner-level validated executor-read and completion-write regions for the accepted completion plan. `Microvmm/Proof/Virtio/RngExecution.lean` separately ties successful completion writes back to the same plan-derived trace, and `Microvmm/Proof/Kvm/{Resource,RawBoundary}.lean` keep the remaining shim/FFI boundary explicit. What is still missing is one single exported theorem packaging those pieces into a full runtime validated-access claim up to the named raw boundary. |
